...
const AppContent: React.FC<{ isMaintenanceMode: boolean }> = ({ isMaintenanceMode }) => {
  ...
  return (
    <div className="relative">
      ...
      {/* Universal top-right cluster — Agencies dropdown + Hit Us CTA, on every page */}
      <div
        className={`fixed top-4 right-4 md:top-6 md:right-6 z-[60] flex items-center gap-2 md:gap-3 transition-opacity duration-300 pointer-events-auto ${hideLogo ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        style={{ animation: 'logoSlideDown 1.4s cubic-bezier(0.34, 1.56, 0.64, 1) 0.3s both' }}
      >
